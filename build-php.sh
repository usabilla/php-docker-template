...
sed -E "s/${IMAGE_ORIGINAL_TAG}/${IMAGE_TAG}/g" "Dockerfile-${IMAGE}" | docker build --platform linux/amd64,linux/arm64 --pull -t "${USABILLA_TAG}" --target="${IMAGE}" -f - . \
    && echo "$USABILLA_TAG" >> "$TAG_FILE"

sed -E "s/${IMAGE_ORIGINAL_TAG}/${IMAGE_TAG}/g" "Dockerfile-${IMAGE}" | docker build --platform linux/amd64,linux/arm64 --pull -t "${USABILLA_TAG_DEV}" --target="${IMAGE}-dev" -f - . \
